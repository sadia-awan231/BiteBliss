	...
	function foodland_site_branding() {
		...
		if ( ('' != $site_title || '' != $site_tagline )  && true == $site_title_enable ){
			$foodland_header_text = '
				<div id="site-header">';
				if( !empty( $site_title ) && ( is_front_page() || is_home() ) ){					
					$foodland_header_text .='<h1 class="site-title"><a href="' . esc_url( home_url( '/' ) ) . '">' . esc_html( $site_title ) . '</a></h1>';
				}else{
					$foodland_header_text .='<p class="site-title"><a href="' . esc_url( home_url( '/' ) ) . '">' . esc_html( $site_title ) . '</a></p>';
				}
